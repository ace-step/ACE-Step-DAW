...
/**
 * Simple mode: user enters a short natural-language description,
 * optionally sets vocal language / instrumental, then clicks "Create Sample"
 * to have the LM infer caption, lyrics, BPM, key, duration, etc.
 *
 * On success, calls onSampleCreated which switches to Custom mode
 * with all fields pre-filled.
 */
export function SimpleModeForm({ onSampleCreated }: SimpleModeFormProps) {
  ...
  const handleCreateSample = useCallback(async () => {
    ...
    try {
      ...
      // Pass result to parent → auto-switch to Custom mode
      onSampleCreated({
  ...
  return (
    <div className="flex flex-1 flex-col overflow-y-auto" data-testid="simple-mode-form">
      ...
      <div className="border-t border-[#3a3a3a] px-4 py-3">
        <Button
          ...
          className="w-full"
          data-testid="simple-mode-create-btn"
        >
