...
export function smoothGain(
  ...
  // One-pole coefficient: alpha = 1 - e^(-dt/tau)
 ...
 *
 * Usage:
 *   const follower = new SidechainFollower(ctx, sourceAnalyserOrNode, params);
 *   // Insert follower.gainNode into the target track's signal chain
 *   someNode.connect(follower.gainNode);
 *   follower.gainNode.connect(nextNode);
 *   // Update params in real-time:
 *   follower.updateParams(newParams);
 *   // Dispose when done:
 *   follower.dispose();
 */
