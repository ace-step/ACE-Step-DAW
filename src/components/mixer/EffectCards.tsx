...
export function EQ3Card({ effect, trackId }: { effect: TrackEffect & { type: 'eq3' }; trackId: string }) {
  ...
  return (
    <EffectCardLayout
      ...
      visualization={<EQCurve low={p.low} mid={p.mid} high={p.high} lowFreq={p.lowFrequency} highFreq={p.highFrequency} />}
      footer={
...
export function EQCurve({ low, mid, high, lowFreq = 400, highFreq = 2500, color = EFFECT_COLORS.eq3 }: { low: number; mid: number; high: number; lowFreq?: number; highFreq?: number; color?: string }) {
  ...
  useEffect(() => {
    ...
    const dbRange = 15; // ±15 dB display range
    ...
    ctx.beginPath();
    ctx.moveTo(lowX, 2);
    ctx.lineTo(lowX, drawH - 2);
    ctx.stroke();
    ctx.beginPath();
    ctx.moveTo(highX, 2);
    ctx.lineTo(highX, drawH - 2);
    ctx.stroke();
    ctx.setLineDash([]);
    ...
    for (let px = 0; px <= W; px++) {
      ...
      // Smooth crossover blending using log-domain distance
      const logF = Math.log10(freq);
      const logLow = Math.log10(lowFreq);
      const logHigh = Math.log10(highFreq);
      const crossoverWidth = 0.5; // octaves of crossover transition

      if (logF <= logLow - crossoverWidth) {
        gain = low;
