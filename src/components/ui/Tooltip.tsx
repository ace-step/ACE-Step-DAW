...
export function Tooltip({
  ...
  const tooltipEl = visible
    ? createPortal(
        ...
        >
          <div className="rounded-md bg-zinc-900 px-2 py-1.5 text-[11px] leading-snug text-zinc-100 daw-shadow-md">
            <span>{content}</span>
            {shortcut && (
              <kbd className="ml-1.5 inline-block rounded border border-zinc-700 bg-zinc-800 px-1 py-0.5 font-mono text-[10px] text-zinc-400">
                {shortcut}
              </kbd>
            )}
          </div>
          ...
          {position && (
            ...
            >
              <svg width={ARROW_SIZE * 2} height={ARROW_SIZE * 2} viewBox="0 0 10 10">
                <polygon points="5,10 0,0 10,0" fill="#18181b" />
              </svg>
            </div>
          )}
        </div>,
