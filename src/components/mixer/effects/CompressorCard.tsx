...
export function CompressorCard({ effect, trackId }: { effect: TrackEffect & { type: 'compressor' }; trackId: string }) {
  ...
  const grRef = useRef({ display: 0, sc: 0, peak: 0, hold: 0 });

  const hasSidechain = !!p.sidechainSourceTrackId;
  const otherTracks = tracks.filter((t) => t.id !== trackId);

  useEffect(() => {
    const smooth = (prev: number, target: number) => prev + (target - prev) * (target > prev ? 0.5 : 0.05);
    const tick = () => {
      const r = grRef.current;
      r.display = smooth(r.display, Math.abs(effectsEngine.getCompressorReduction(trackId, effect.id)));
      r.sc = smooth(r.sc, Math.abs(effectsEngine.getSidechainReduction(trackId, effect.id)));
      if (r.display > r.peak) { r.peak = r.display; r.hold = 90; }
      else if (r.hold > 0) r.hold--;
      else r.peak = Math.max(0, r.peak - 0.3);
      setReduction(-r.display);
      setScReduction(-r.sc);
      setPeakGr(r.peak);
